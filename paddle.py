...
font1 = pg.font.Font('arial.ttf', 45)
...
def checkcollision():
    global SCREEN_WIDTH,paddle,BALL_X,BALL_Y,BALL_X_SPEED,BALL_Y_SPEED,GAME_STARTED,Color
...
pg.display.set_caption('paddle ball')
...
while True:
    ...
    if Game_Running==True:
        ...
        window.fill("#d6c1e8")   
        pg.draw.rect(window,"#154c79",paddle)
        
        if GAME_STARTED==False:
            pg.draw.circle(window,"black",(BALL_X,BALL_Y),8)       
        else:
            BALL_X+=BALL_X_SPEED*dt*TARGET_FPS
            BALL_Y+=BALL_Y_SPEED*dt*TARGET_FPS
            pg.draw.circle(window,"dark green",(BALL_X,BALL_Y),8)
